...
def calculate_duo(token_1,token_2):
    for first_trade_market in MARKETS:
        for second_trade_market in MARKETS:
            try:
                ...
                token_2_total = Decimal(trade_1[1]) * 5
                token_1_end_total = token_2_total * Decimal(trade_2[1])
                pos_or_neg = 5 - token_1_end_total
                print(f'{token_2_total}')
                print(f' 5 - {token_1_end_total}')
                print (pos_or_neg)
        ...
        for second_trade_market in MARKETS:
            for third_trade_market in MARKETS:
                try:
                    ...
                    forward_calculation = ((Decimal(trade_1[1])) * (1 / Decimal(trade_2[0])) * (1 / Decimal(trade_3[0]))) - 1
                    forward_calculation_percent = forward_calculation * 100
                    return (f'{first_trade_market},{second_trade_market},{third_trade_market},{token1},{token2},{token3}', forward_calculation)
                except Exception as e:
                    if "ZRX" in [token1, token2, token3]:
                        return (f'{token1},{token2},{token3}', -1000000)
                    if "OMG" in [token1, token2, token3]:
            ...
            for second_trade_market in MARKETS:
                for third_trade_market in MARKETS:
                    try:
                        ...
                        return f'{token1},{token2},{token3}', reversal_calculation
                    except Exception as e:
                        if "ZRX" in [token1, token2, token3]:
                            return (f'{token1},{token2},{token3}', -1000000)
                        if "OMG" in [token1, token2, token3]:
