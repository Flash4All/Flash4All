   ...
   for first_trade_market in MARKETS:
       ...
       for second_trade_market in MARKETS:
           if token3 is None: 
               ...
               token_2_amount = Decimal(trade_1[1]) * 10
               simple_flip_flop = ((Decimal(trade_2[1]) * token_2_amount)/10 - 1)
               results.append((f'{first_trade_market},{second_trade_market},{token1},{token2}', simple_flip_flop))
